...
holland_current <- function(Season = 2017) {
    ...
    n1 = read_csv(paste0("http://www.football-data.co.uk/mmz4281/", s2, s1, "/N1.csv"))
    df1 <- bind_rows(getCurrentData(n1, "N1", 1, Season = myseason))
    df1$Date <- as.Date(df1$Date, format = "%Y-%m-%d")
    holl <- holland
    if (identical(max(df1$Date), max(holl$Date)))
        warning("The returned dataframe contains data already included in 'holland' dataframe")
    tm <- teamnames
