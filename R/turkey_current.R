...
turkey_current <- function(Season = 2017) {
    ...
    t1 = read_csv(paste0("http://www.football-data.co.uk/mmz4281/", s2, s1, "/T1.csv"))
    df1 <- bind_rows(getCurrentData(t1, "T1", 1, Season = myseason))
    df1$Date <- as.Date(df1$Date, format = "%Y-%m-%d")
    turk <- turkey
    if (identical(max(df1$Date), max(turk$Date)))
        warning("The returned dataframe contains data already included in 'turkey' dataframe")
    tm <- teamnames
