...
spain_current <- function(Season = 2017) {
    ...
    sp1 = read_csv(paste0("http://www.football-data.co.uk/mmz4281/", s2, s1, "/SP1.csv"))
    df1 <- bind_rows(getCurrentData(sp1, "SP1", 1, Season = myseason))
    df1$Date <- as.Date(df1$Date, format = "%Y-%m-%d")
    spai <- spain
    if (identical(max(df1$Date), max(spai$Date)))
        warning("The returned dataframe contains data already included in 'spain' dataframe")
    tm <- teamnames
