...
homeaway <- function(df) {
    ...
    if ("division" %in% colnames(df)) {
        rbind(
            df %>%
                dplyr::select(Date, Season, team = home,
                          opp = visitor, gf = hgoal,
                          ga = vgoal, division, tier) %>%
                dplyr::mutate(venue = "home"),
            df %>%
                dplyr::select(Date, Season, team = visitor,
                          opp = home, gf = vgoal,
